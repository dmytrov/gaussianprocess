...
from validation.crossvalidate_temporal_mp import *
...
if __name__ == "__main__":
    for dataset_id in (1, 2):
        ...
        for key in ["ELBO", "MSE", "WRAP_DYN", "WRAP_PATH", "timing"]:
            ...
            plt.plot(values_by_name(axes, "numprim"), data, "x", markersize=10)
            plt.errorbar(values_by_name(axes, "numprim"), means, std, fmt='--o', capsize=2)
            plt.xlabel("numprim")
            plt.ylabel(key)
            plt.title("TMP, {}".format(key))
            plot_dir = "{}/statistics".format(save_dir)
            if not os.path.exists(plot_dir):
                os.makedirs(plot_dir)
            plt.savefig("{}/TMP-{}.pdf".format(plot_dir, key))
            #plt.show()
            plt.close(fig)
